...
def alg_mine(filename):
    ...
    for row in rows:   # read spread sheet row into object
        stock_inf_obj = RowOfStockinf(
            row[0], row[1], row[2], row[3], row[4], row[5], row[6])  # create Object is a row on the spreadsheet
        ROWSOFSTOCKINF.append(stock_inf_obj)   # List container to hold a series of objects  [0]....[4xxx]
    csvfile.close()
#   ROWSOFSTOCKINF[xxxx]
# len(ROWSOFSTOCKINF).... List ... len(nameOfthelsit)
    # the trading is on the 21 days counting backward
# Buy $2000 of stock base on the the last row's lowest price

# loop the margin here ... buyMargin[0]...[3]... 3%, 5%, 7%, 9%
    ...
    #<------ BLOCK FOR SET FOR A buyMargin
        for index in range(len(ROWSOFSTOCKINF) - 2, 1, -1):    # means this loop start from 4xxx down to 1, -1 each time
            ...
            if RowOfStockinf.cash[x] == 0.0:      # means there are stock to sell
                if priceTosell > toDaylowPrice:               # do my selling
                    ...
                    missingSellcounter = 0  #<--- this one never being use ..... 3563.... have the stock 4xxx-3563
                    RowOfStockinf.numOfSales[x] =RowOfStockinf.numOfSales[x]+1
                else:
                    missingSellcounter = missingSellcounter + 1  # ..... missingSellcounter on the right is "3" and how become "4"
            if RowOfStockinf.numOfStock[x] == 0.0:
                if priceToBuy < toDayHighPrice and priceToBuy > toDaylowPrice:  # in the trading range
                    # do my buy
                    RowOfStockinf.numOfStock[x] =  RowOfStockinf.cash[x]/ priceToBuy
                    RowOfStockinf.cash[x] = 0.0
                    priceTosell = priceToBuy + 100/RowOfStockinf.numOfStock[x]  # present $100 profit
                    missingBuycounter = 0
